...
import 'package:flutter_app_xmly/UI/broad_cast_list_page/broad_cast_list_page.dart';
...
class _HomeBroadCastPageState extends State<HomeBroadCastPage>
  ...
  Widget buildGridViewChannel() {
    List imageArray =
        this.entity.data.radioSquareResults.map((model) => model.icon).toList();
    List titleArray = this
        .entity
        .data
        .radioSquareResults
        .map((model) => model.title)
        .toList();
    return HomeBroadCastHotChannelHeaderView(imageArray, titleArray);
  }

  // 创建列表item
  List<Widget> buildTVItems() {
    return (isExpaned == false
            ? unexpandedChannelTitles
            : expandedChannelTitles)
        .map((title) {
  ...
  Widget creatGridTVChannels() {
    List<String> dataSource =
        isExpaned ? this.expandedChannelTitles : this.unexpandedChannelTitles;
            ...
            onTap: () {
              ...
              if (channel == kExpandChannelName) {
                ...
                Navigator.of(context).push(MaterialPageRoute(builder: (_) {
                  return BroadCastDetailListPage();
                }));
              }
            },
            child: Container(
                ...
                child: Center(
                    child: (channel == kExpandChannelName ||
                            channel == kUnExpandChannelName)
                        ? (channel == kExpandChannelName
                            ? Image.asset("images/arrows_bottom.png")
                            : Image.asset("images/arrows_top.png"))
                        : Text(channel))),
          );
        });
  }
  ...
  Future<String> _loadData() async {
    return DefaultAssetBundle.of(context)
        .loadString("config/home_broadcast_data.json");
  }
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.done) {
            ...
            List<String> allChannels =
                this.entity.data.categories.map((model) => model.name).toList();
              ...
              slivers: <Widget>[
                ...
                SliverToBoxAdapter(
                  child: Container(
                    ...
                    height: 44.0,
                    child:
                        createMoreChannelWidgetItem(this.entity.data.location),
                  ),
                ),
                SliverFixedExtentList(
                  delegate: SliverChildBuilderDelegate(
                    (ctx, index) {
                      HomeBroadcastDataLocalradio model =
                          this.entity.data.localRadios[index];
                      return buildListViewItem(model.coverLarge, model.name,
                          model.programName, model.playCount);
                    },
                ...
                SliverFixedExtentList(
                  delegate: SliverChildBuilderDelegate(
                    (ctx, index) {
                      HomeBroadcastDataTopradio model =
                          this.entity.data.topRadios[index];
                      return buildListViewItem(model.coverLarge, model.name,
                          model.programName, model.playCount);
                    },
